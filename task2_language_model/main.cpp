...
struct vocab_list
{
	...
	void print()
	{
		...
		{
			if (iterator->list->character == ' ')
				printf("\t<<SP>,%d>\n", iterator->list->occurence);
			else
				printf("\t<%c,%d>\n", iterator->list->character, iterator->list->occurence);
			iterator->list = iterator->list->next;
		}
		if (iterator->list->next == NULL && iterator->list->character != ' ' && iterator->list->character != '\0')
			printf("\t<%c,%d>\n", iterator->list->character, iterator->list->occurence);
		else if (iterator->list->character == ' ')
			printf("\t<<SP>,%d>\n", iterator->list->occurence);
		else if (iterator->list->character == '\0')
			printf("\t<<SP>,%d>\n", iterator->list->occurence);
	...
	void add_char(char nextchar)
	{
		...
		if (nextchar >= 'a' && nextchar <= 'z')
		{
			...
			{
				while (iterator->next != NULL && iterator->next->character < nextchar)
					iterator = iterator->next;
		...
		{
			while (iterator->next != NULL && iterator->character != nextchar)
				iterator = iterator->next;
	...
	void add_occurence(char c1, char c2)
	{
		...
		if (c2 == '\n')
			c2 = ' ';
		while (iterator->next != NULL && iterator->character != c1)
			iterator = iterator->next;
		...
		{
			while (iterator->list->next != NULL && iterator->list->character != c2)
				iterator->list = iterator->list->next;
			if (iterator->list->character == c2)
				iterator->list->occurence++;
			else
			{
				occur_node *add = (occur_node *)malloc(sizeof(occur_node));
	...
	int get_occurence(char c)
	{
		int counter = 0;
		iterator->list = iterator->root;
		while (iterator->next != NULL && iterator->character != c)
			iterator = iterator->next;
		if (iterator->list->next == NULL && iterator->character == c)
...
int main(int argc, char **argv)
{
	...
	{ // if only the file name is entered
		getinput->readData(argv[1]);
		while (getinput->vocabularylist->iterator->next != NULL)
			getinput->vocabularylist->print();
	}
